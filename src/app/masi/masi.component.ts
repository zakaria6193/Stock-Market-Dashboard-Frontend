...
@Component({ templateUrl: 'masi.component.html' })
      ...
      public getfrombase(): void {
        ...
        this.Dash.frombasemasi().subscribe(
          (response: any) => {
            ...
            this.tuniprice=this.resp.masiprice[0]
            this.tuniopen=this.resp.masiopen[0]
            this.tunichangeperc=this.resp.masichangeperc[0]
            this.tunivolume=this.resp.masivolume[0]
            this.tunichange=this.resp.masichange[0]
...
public gethistorical(symb:string){
  this.spinner.show();
  this.Dash.realhistoricalmasi(symb).subscribe(
    (response: any) => {
...
public onSearch(symb:string){
  this.spinner.show();
  this.Dash.realhistoricalmasi(symb.toUpperCase()).subscribe(
    (response: any) => {
      sessionStorage.clear();
      sessionStorage.setItem('symbole', symb.toUpperCase());
...
public Multiplelines(symb:string){
  ...
  this.Dash.listhistoricalmasi(lstsymb).subscribe(
    (response: any) => {
...
public MAprocess(MA:any){
  ...
  this.Dash.MAgetmasi(MA.symb,MA.per).subscribe(
    (response: any) => {
...
public MACDprocess(MACD:any){
  ...
  this.Dash.MACDgetmasi(MACD.symb,MACD.fast,MACD.slow,MACD.single).subscribe(
    (response: any) => {
...
public EMAprocess(EMA:any){
  ...
  this.Dash.EMAgetmasi(EMA.symb,EMA.per).subscribe(
    (response: any) => {
