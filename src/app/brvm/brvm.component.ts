...
@Component({ templateUrl: 'brvm.component.html' })


export class BrvmComponent implements OnInit{
      ...
      public getfrombase(): void {
        ...
        this.Dash.frombasebrvm().subscribe(
          (response: any) => {
            ...
            this.tuniprice=this.resp.brvmprice[0]
            this.tuniopen=this.resp.brvmopen[0]
            this.tunichangeperc=this.resp.brvmchangeperc[0]
            this.tunivolume=this.resp.brvmvolume[0]
            this.tunichange=this.resp.brvmchange[0]
...
public gethistorical(symb:string){
  this.spinner.show();
  this.Dash.realhistoricalbrvm(symb).subscribe(
    (response: any) => {
...
public onSearch(symb:string){
  this.spinner.show();
  this.Dash.realhistoricalbrvm(symb.toUpperCase()).subscribe(
    (response: any) => {
      sessionStorage.clear();
      sessionStorage.setItem('symbole', symb.toUpperCase());
...
public Multiplelines(symb:string){
  ...
  this.Dash.listhistoricalbrvm(lstsymb).subscribe(
    (response: any) => {
...
public MAprocess(MA:any){
  ...
  this.Dash.MAgetbrvm(MA.symb,MA.per).subscribe(
    (response: any) => {
...
public MACDprocess(MACD:any){
  ...
  this.Dash.MACDgetbrvm(MACD.symb,MACD.fast,MACD.slow,MACD.single).subscribe(
    (response: any) => {
...
public EMAprocess(EMA:any){
  ...
  this.Dash.EMAgetbrvm(EMA.symb,EMA.per).subscribe(
    (response: any) => {
